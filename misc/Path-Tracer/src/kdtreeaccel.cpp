...
// allocate aligned meomery is OS specified.
void* AllocAligned(size_t size)
{
#if defined(OS_IS_WINDOWS)
	return _aligned_malloc(size, L1_CACHE_LINE_SIZE);
#elif defined(OS_IS_OPENBSD) || defined(OS_IS_OSX) || defined(OS_IS_FREEBSD)
	void *ptr;
...
retrySplit:
    ...
    for (int i = 0; i < 2 * nPrimitives; ++i)
	{
		...
		{
            // Compute cost for split at _i_th edgeT
    ...
    nodes[nodeNum].InitInterior(bestAxis, aboveChild, tSplit); // Current is an interior node.
