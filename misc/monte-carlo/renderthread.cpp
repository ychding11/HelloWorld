...
void RenderThread::monteCarloIntegration(short curveIndex, float &X, float &Y, float &Z) 
{
    ...
    S *= (lambdaMax - lambdaMin) / fSamples; 
    // integral = (b-a) * 1/N * sum_{i=0}^{N-1} f(X_i) and normalize
    X *= (lambdaMax - lambdaMin) / fSamples / S; 
    Y *= (lambdaMax - lambdaMin) / fSamples / S; 
    Z *= (lambdaMax - lambdaMin) / fSamples / S; 
}

void saveToFile(float *pixels, int width, int height) 
{ 
    std::ofstream ofs; 
    ofs.open("./mcbeth.ppm"); 
    ofs << "P6\n" << width << " " << height << "\n255\n"; 
...
int RenderThread::tester(void) 
{
	constexpr int32_t squareSize = 64;
	constexpr int32_t width = squareSize * 6, height = squareSize * 4; // 6 columns, 4 rows
	...
	memset(buffer, 0x00, sizeof(float) * width * height * 3);
	...
	{ 
            pixels[i * 3 + 0] = powf(buffer[i * 3 + 0] / fNumPass, fGamma);
            pixels[i * 3 + 1] = powf(buffer[i * 3 + 1] / fNumPass, fGamma);
            pixels[i * 3 + 2] = powf(buffer[i * 3 + 2] / fNumPass, fGamma);
        } 
...
{
	printf("render threading is constructing...\n");
}

RenderThread::~RenderThread()
{
	printf("render threading is destructing...\n");
}
