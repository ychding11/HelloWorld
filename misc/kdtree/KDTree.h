 ...
 * destruct all nodes in a tree.
 * It is post-order traversal
 */
template <size_t N, typename ElemType>
...
/* 
 * Copy constructor 
 * NOTE: input parameter is const reference
 */
template <size_t N, typename ElemType>
 ...
 * Clears old tree if not the same tree and then
 * copies the "other" tree into the new tree
 */
template <size_t N, typename ElemType>
KDTree<N, ElemType>& KDTree<N, ElemType>::operator=(const KDTree& rhs) {
    if (this != &rhs) {
        deleteNode(this->root); //destruct current tree
        
        root = copyTree(rhs.root); //construct a new tree by copy
        numElements = rhs.numElements;
    }
    return *this;
}
...
/*
 * copyTree(rootNode)
 * Private function copy one tree from another. It uses a recursive method
